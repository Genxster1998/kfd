...
// (This temporarily overwrites tccd, and on iOS <=14, changes do not revert on reboot)
...
static bool overwrite_file(int fd, NSData* sourceData) {
    ...
    uint64_t openedfile = kread64(filedesc + (8 * fd));
    ...
    printf("[i] overwrite_file vnode: 0x%llx\n", to_vnode);
    ...
    printf("[i] overwrite_file vnode->v_writecount: %d\n", to_vnode_v_writecount);
